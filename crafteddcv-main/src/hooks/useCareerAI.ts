...
export function useCareerAI() {
  ...
  const analyzeJobMatch = (cv: CVFormData, jobDescription: string) =>
  ...
  const generateInterviewPrep = async (cv: CVFormData, jobDescription: string) => {
  ...
  const suggestSkills = async (cv: CVFormData, jobDescription?: string, targetJobTitle?: string) => {
    const result = await invoke<{ skills: string[] }>("suggest_skills", cv, jobDescription || "", { targetJobTitle });
